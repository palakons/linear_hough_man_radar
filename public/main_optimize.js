...
// Compose two transforms: first (r1, t1), then (r2, t2)
function transformWithOptimizedRT(point, curRot, curTrans, optRot, optTrans) {
    // Apply current rotation/translation
    let pt = transformToWorld(point, curRot, curTrans);
    // Apply optimized rotation/translation
    pt = transformToWorld(pt, optRot, optTrans);
    return pt;
}

// Function to transform all points and lines in data
function transformDataToWorld(data, sensorInfo, optRT = null) {
    ...
    transformedData.points = data.points.map(point => {
        if (optRT) {
            return transformWithOptimizedRT(point, sensorInfo.rotation, sensorInfo.translation, optRT.rotation, optRT.translation);
        } else {
            return transformToWorld(point, sensorInfo.rotation, sensorInfo.translation);
        }
    });
    ...
    transformedData.lines = data.lines.map(line => {
        ...
        if (optRT) {
            transformedLine.p0 = transformWithOptimizedRT(line.p0, sensorInfo.rotation, sensorInfo.translation, optRT.rotation, optRT.translation);
            transformedLine.p1 = transformWithOptimizedRT(line.p1, sensorInfo.rotation, sensorInfo.translation, optRT.rotation, optRT.translation);
            transformedLine.lineP0 = transformWithOptimizedRT(line.lineP0, sensorInfo.rotation, sensorInfo.translation, optRT.rotation, optRT.translation);
            transformedLine.lineP1 = transformWithOptimizedRT(line.lineP1, sensorInfo.rotation, sensorInfo.translation, optRT.rotation, optRT.translation);
            transformedLine.inlierCloud = line.inlierCloud.map(point =>
                transformWithOptimizedRT(point, sensorInfo.rotation, sensorInfo.translation, optRT.rotation, optRT.translation)
            );
        } else {
...
function refreshRadarDisplay() {
    radarSensors.forEach(sensor => {
        if (currentRadarData[sensor]) {
            ...
            if (getCurrentCoordSystem() === 'world' && currentFrameData && currentFrameData[sensor]) {
                displayData = transformDataToWorld(currentRadarData[sensor], currentFrameData[sensor], optimizedRT && document.getElementById('applyOptimizedRT').checked ? optimizedRT['steps'][currentRTStep][sensor] : null);
            }
...
// Load optimized r/t from JSON (all time steps)
let optimizedRT = null;
let currentRTStep = 0;
let rtStepsCount = 0;

function updateRTTimeStepUI() {
    const slider = document.getElementById('rtTimeStepSlider');
    const valueSpan = document.getElementById('rtTimeStepValue');
    const mainLossDiv = document.getElementById('rtMainLoss');
    const regLossDiv = document.getElementById('rtRegLoss');
    if (!slider || !optimizedRT || !optimizedRT.steps) return;
...
function getOptimizedRTForStep(stepIdx) {
    if (!optimizedRT || !optimizedRT.steps) return null;
    // Return {radar: {rotation, translation}} for this step
    const step = optimizedRT.steps[stepIdx];
    let rt = {};
    for (const radar in step.r) {
        rt[radar] = {
            rotation: step.r[radar],
            translation: step.t[radar]
        };
    }
    return rt;
}

function applyOptimizedRTStep(stepIdx) {
    // Use getOptimizedRTForStep(stepIdx) for r and t
    const rt = getOptimizedRTForStep(stepIdx);
    if (!rt) return;
...
fetch('/r_t_optimized.json').then(r => r.json()).then(json => {
    ...
    console.log(`Total RT steps: ${rtStepsCount}`);
    // Initialize currentRTStep to 0
    updateRTTimeStepUI();
    applyOptimizedRTStep(currentRTStep);
}).catch(() => { optimizedRT = null; });

document.getElementById('rtTimeStepSlider').addEventListener('input', function (e) {
    currentRTStep = parseInt(e.target.value);
    updateRTTimeStepUI();
